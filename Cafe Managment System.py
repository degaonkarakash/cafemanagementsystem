...
#Frames are created
Tops = Frame(root, width = 800, height = 70, bd = 8,background="brown",relief = "raise")
Tops.pack(side = TOP)

f1 = Frame(root, width = 500, height = 400, bd = 0, relief = "raise")
f1.pack(side = LEFT)

f2 = Frame(root, width = 250, height = 300, bd = 8, relief = "raise")
f2.pack(side = RIGHT)


f1a = Frame(f1, width = 0, height = 150, bd = 8, relief = "raise")
f1a.pack(side = TOP)

f2a = Frame(f1, width = 250, height = 160, bd = 6, relief = "raise")
f2a.pack(side = BOTTOM)

ft2 = Frame(f2, width = 200, height = 250, bd = 12, relief = "raise")
ft2.pack(side = TOP)
fb2 = Frame(f2, width = 200, height = 90, bd = 16, relief = "raise")
fb2.pack(side = BOTTOM)

f1aa = Frame(f1a, width = 200, height = 150, bd = 16, relief = "raise")
f1aa.pack(side = LEFT)
f1ab = Frame(f1a, width = 200, height = 150, bd = 16, relief = "raise")
f1ab.pack(side = RIGHT)

f2aa = Frame(f2a, width = 250, height = 150, bd = 14, relief = "raise")
f2aa.pack(side = LEFT)

f2ab = Frame(f2a, width = 250, height = 150, bd = 6, relief = "raise")
...
lblInfo = Label(Tops, font  = ('blackadder ITC', 50, 'bold'), text = " \t Starbucksʨ\t\t", bd = 50,foreground='lime')
...
def CostofItems():
    ...
    PriceofDrinks = (Item1 * 25) + (Item2 * 70) + (Item3 * 60) + (Item4 * 70) + (Item5 * 100) + (Item6 * 90) + (Item7 * 90) + (Item8 * 80)
    PriceofCakes =  (Item9 * 110) + (Item10 * 125) + (Item11 * 120) + (Item12 * 140) + (Item13 * 180) + (Item14 * 160) + (Item15 * 220) + (Item16 * 350)
...
def Receipt():
    ...
    if E_Tea.get() > 0:
        txtReceipt.insert(END, 'Latte:\t\t\t'+str(E_Tea.get())+'\t\t\t'+str('Rs%.2f'%(E_Tea.get()*25))+'\n')
    if E_Espresso.get() > 0:
        txtReceipt.insert(END, 'Espresso:\t\t\t'+str(E_Espresso.get())+'\t\t\t'+str('Rs%.2f'%(E_Espresso.get()*70))+'\n')
    if E_Iced_Latte.get() > 0:
        txtReceipt.insert(END, 'Iced Latte:\t\t\t'+str(E_Iced_Latte.get())+'\t\t\t'+str('Rs%.2f'%(E_Iced_Latte.get()*60))+'\n')
    if E_Vale_Coffee.get() > 0:
        txtReceipt.insert(END, 'Vale Coffee:\t\t\t'+str(E_Vale_Coffee.get())+'\t\t\t'+str('Rs%.2f'%(E_Vale_Coffee.get()*70))+'\n')
    if E_Cappuccino.get() > 0:
        txtReceipt.insert(END, 'Cappuccino:\t\t\t'+str(E_Cappuccino.get())+'\t\t\t'+str('Rs%.2f'%(E_Cappuccino.get()*100))+'\n')
    if E_African_Coffee.get() > 0:
        txtReceipt.insert(END, 'African Coffee:\t\t\t'+str(E_African_Coffee.get())+'\t\t\t'+str('Rs%.2f'%(E_African_Coffee.get()*90))+'\n')
    if E_American_Coffee.get() > 0:
        txtReceipt.insert(END, 'American Coffee:\t\t\t'+str(E_American_Coffee.get())+'\t\t\t'+str('Rs%.2f'%(E_American_Coffee.get()*90))+'\n')
    if E_Iced_Cappuccino.get() > 0:
        txtReceipt.insert(END, 'Iced Cappuccino:\t\t\t'+str(E_Iced_Cappuccino.get())+'\t\t\t'+str('Rs%.2f'%(E_Iced_Cappuccino.get()*80))+'\n')
    if E_Coffee_Cakes.get() > 0:
        txtReceipt.insert(END, 'Coffee Cake:\t\t\t'+str(E_Coffee_Cakes.get())+'\t\t\t'+str('Rs%.2f'%(E_Coffee_Cakes.get()*110))+'\n')
    if E_Red_Velvet_Cake.get() > 0:
        txtReceipt.insert(END, 'Red Velvet Cake:\t\t\t'+str(E_Red_Velvet_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Red_Velvet_Cake.get()*125))+'\n')
    if E_Black_Forest_Cake.get() > 0:
        txtReceipt.insert(END, 'Black Forest Cake:\t\t\t'+str(E_Black_Forest_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Black_Forest_Cake.get()*120))+'\n')
    if E_Boston_Cream_Cake.get() > 0:
        txtReceipt.insert(END, 'Boston Cream Cake:\t\t\t'+str(E_Boston_Cream_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Boston_Cream_Cake.get()*140))+'\n')
    if E_Lagos_Chocolate_Cake.get() > 0:
        txtReceipt.insert(END, 'Lagos Chocolate Cake:\t\t\t'+str(E_Lagos_Chocolate_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Lagos_Chocolate_Cake.get()*180))+'\n')
    if E_Kilburn_Chocolate_Cake.get() > 0:
        txtReceipt.insert(END, 'Kilburn Chocolate Cake:\t\t\t'+str(E_Kilburn_Chocolate_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Kilburn_Chocolate_Cake.get()*160))+'\n')
    if E_Carlton_Hill_Chocolate_Cake.get() > 0:
        txtReceipt.insert(END, 'Carlton Hill Chocolate Cake:\t\t\t'+str(E_Carlton_Hill_Chocolate_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Carlton_Hill_Chocolate_Cake.get()*220))+'\n')
    if E_Queen_Park_Chocolate_Cake.get() > 0:
        txtReceipt.insert(END, 'Queen Park Chocolate Cake:\t\t\t'+str(E_Queen_Park_Chocolate_Cake.get())+'\t\t\t'+str('Rs%.2f'%(E_Queen_Park_Chocolate_Cake.get()*350))+'\n')
